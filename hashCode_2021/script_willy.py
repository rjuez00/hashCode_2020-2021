...
inputs = ['a', 'b', 'c', 'd', 'e', 'f']
for inp in inputs:
    with open("Aprueba" + inp, "w") as file:
        parse = inputParser(inp + ".txt")
        ...
        for i in range(len(parse.intersectionData)):
            if(len(parse.intersectionData[i]) != 0):
                ...
                for calle in parse.intersectionData[i]:
                    if parse.streetData[calle]['count'] != 0:
                        streets_with_car += 1
                        if parse.streetData[calle]['count']/parse.streetData[calle]["time"] < min_cars:
                            min_cars = parse.streetData[calle]['count']/parse.streetData[calle]["time"]
                if streets_with_car != 0:
                    file.write(str(i) + "\n")
                    file.write(str(streets_with_car) + "\n")
                    for calle in parse.intersectionData[i]:
                        if parse.streetData[calle]['count'] != 0:
                            file.write(calle + " " + str(math.floor((parse.streetData[calle]['count']/parse.streetData[calle]["time"])/min_cars)) + "\n")
